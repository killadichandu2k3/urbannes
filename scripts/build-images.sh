...
build_and_load() {
  ...
  local dockerfile="$2"
  echo "==> Building ${name}"
  docker build -f "${dockerfile}" -t "urbannest/${name}:latest" .
  echo "==> Loading ${name} into Kind cluster '${CLUSTER_NAME}'"
  kind load docker-image "urbannest/${name}:latest" --name "${CLUSTER_NAME}"
}
...
docker build -f gateway/Dockerfile -t urbannest/gateway:latest .
echo "==> Loading gateway into Kind cluster '${CLUSTER_NAME}'"
kind load docker-image urbannest/gateway:latest --name "${CLUSTER_NAME}"

echo "==> All images built and loaded into Kind."
docker images | grep urbannest
